...
class ReportHandler {
  ...
  private async fetchWorkspaceData(
    userId: string,
    folderId?: string
  ): Promise<{ sections: ReportSection[]; totalItems: number }> {
  ...
  private formatFolderSection(folder: any, parentName?: string): ReportSection {
    ...
    if (
      folder.name.toLowerCase().includes('financeiro') ||
    ...
    return {
      title: fullName,
      type: 'list',
      content: folder.items.map((item: any) => ({
        title: item.title,
  ...
  private formatFinancialTable(title: string, items: any[]): ReportSection {
    const rows = items.map((item: any) => {
      const content = item.content;
      const valor =
        content.valor || content.preco || content.preço || content.total || 0;
      return [
        item.title,
        content.fornecedor ||
        content.responsavel ||

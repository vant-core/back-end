...
export class ReportNarrativeRules {
  // Função principal que gera o resumo executivo
  generateExecutiveSummary(input: ExecutiveSummaryInput): ReportSection {
    ...
    // Introdução
    paragraphs.push(
      `Este relatório apresenta uma análise consolidada das informações registradas no workspace${
        title ? `, com foco em "${title}"` : ''
      }. O objetivo é oferecer uma visão clara do estágio atual de planejamento e organização.`
    );

    // Volume de dados
    if (totalItems > 0) {
      paragraphs.push(
        `Foram analisados ${totalItems} registro${
          totalItems > 1 ? 's' : ''
        } distribuídos entre diferentes categorias, incluindo eventos, informações financeiras e listas operacionais.`
      );
    }
    ...
    if (hasEvents) {
      paragraphs.push(
        `As seções relacionadas a eventos permitem compreender o calendário, os locais envolvidos e a dimensão de público esperada, apoiando decisões sobre logística, equipe e comunicação.`
      );
    }

    if (hasFinance) {
      paragraphs.push(
        `Os dados financeiros consolidam valores por fornecedor e status, oferecendo uma leitura objetiva sobre compromissos assumidos, pendências e controle orçamentário.`
      );
    }

    if (hasLists) {
      paragraphs.push(
        `Listas complementares organizam tarefas, anotações e informações auxiliares, mantendo o contexto operacional acessível e estruturado.`
      );
    }

    // Encerramento estratégico
    paragraphs.push(
      `Em conjunto, essas informações fornecem uma base sólida para acompanhamento do projeto, alinhamento com stakeholders e definição de próximos passos.`
    );
  ...
  generateSectionContext(section: ReportSection): ReportSection | null {
  ...
  private generateTableContext(section: ReportSection): string {
    ...
    let text = `Esta seção consolida ${rows.length} registro${
      rows.length > 1 ? 's' : ''
    }, permitindo uma análise objetiva dos dados apresentados. `;
  ...
  private generateListContext(section: ReportSection): string {
    const items = section.content || [];
    const total = items.length;

    let text = `Esta seção organiza ${total} registro${
      total > 1 ? 's' : ''
    } em formato de lista, relacionados a "${section.title}". `;

    const withDescription = items.filter(i => i.description).length;

    if (withDescription > 0) {
      text += `${withDescription} item${
        withDescription > 1 ? 's possuem' : ' possui'
      } descrição detalhada, enriquecendo o contexto e facilitando a compreensão. `;
    }
